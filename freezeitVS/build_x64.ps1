$ErrorActionPreference = 'Stop'

function log
{
    [CmdletBinding()]
    Param
    (
        [Parameter(Mandatory = $true, Position = 0)]
        [string]$LogMessage
    )
    Write-Output ("[{0}] {1}" -f (Get-Date), $LogMessage)
}

$windowsToolchainsDir = "C:/Microsoft/AndroidNDK/android-ndk-r23c/toolchains/llvm/prebuilt/windows-x86_64/bin"
$clang = "${windowsToolchainsDir}/clang++.exe"
#$target = "--target=aarch64-none-linux-android29"
$target = "--target=x86_64-none-linux-android29"
$sysroot = "--sysroot=C:/Microsoft/AndroidNDK/android-ndk-r23c/toolchains/llvm/prebuilt/windows-x86_64/sysroot"
$cppFlags = "-std=c++20 -static -s -Ofast -Wall -Wextra -Wshadow -fno-rtti -DNDEBUG -fPIE"

log "Compiler..."
& $clang $target $sysroot $cppFlags.Split(' ') -I. main.cpp -o ./X64/freezeit

log "Done"
